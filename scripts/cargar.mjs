...
const ubic=[]; U.eachRow((r,n)=>{if(n===1)return;const sede=String(r.getCell(iu['sede']).value??'').trim();const piso=String(r.getCell(iu['piso']).value??'').trim();const etiqueta=String(r.getCell(iu['tipo']).value??'').trim();const area=String(r.getCell(iu['etiqueta']).value??'').trim();if(!sede||!piso||!etiqueta)return;ubic.push({sede,piso,area:area||null,tipo:tipoDe(etiqueta),etiqueta,cupo_default:num(r.getCell(iu['cupo']).value)||1,orden:num(r.getCell(iu['orden']).value),activo:siNo(r.getCell(iu['activo']).value)})})

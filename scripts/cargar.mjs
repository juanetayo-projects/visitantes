...
const ubic=[]; U.eachRow((r,n)=>{if(n===1)return;const sede=String(r.getCell(iu['sede']).value??'').trim();const piso=String(r.getCell(iu['piso']).value??'').trim();const etiqueta=String(r.getCell(iu['tipo']).value??'').trim();const area=String(r.getCell(iu['area']).value??'').trim();const servicio=String(r.getCell(iu['etiqueta']).value??'').trim();if(!sede||!piso||!etiqueta)return;ubic.push({sede,piso,area:area||null,servicio:servicio||null,tipo:tipoDe(etiqueta),etiqueta,cupo_default:num(r.getCell(iu['cupo']).value)||1,orden:num(r.getCell(iu['orden']).value),activo:siNo(r.getCell(iu['activo']).value)})})
...
const payloadU=ubic.map(u=>({piso_id:pk.get(sid.get(norm(u.sede))+'|'+norm(u.piso)),area:u.area,servicio:u.servicio,tipo:u.tipo,etiqueta:u.etiqueta,cupo_default:u.cupo_default,orden:u.orden,activo:u.activo})).filter(u=>u.piso_id)

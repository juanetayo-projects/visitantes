...
export default function Monitoreo() {
  ...
  return (
    <div style={{ background: '#081831', borderRadius: 16, padding: 16, color: '#E6F1FB', height: 'calc(100vh - 100px)', display: 'flex', flexDirection: 'column', gap: 10, overflow: 'hidden' }}>
      {/* Top bar */}
      ...
      {/* KPIs */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2.5 flex-none">
      ...
      {/* Heatmap + feed — ocupa el espacio restante */}
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1.6fr) minmax(0,1fr)', gap: 12, flex: 1, minHeight: 0 }}>
        <div style={{ ...PANEL, padding: 14, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          ...
          />
          <div className="flex items-center gap-1.5 mt-2 flex-none" style={{ fontSize: 10, color: '#5b7aa6' }}>
            menos
        ...
        <div style={{ ...PANEL, padding: 14, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ fontSize: 13, fontWeight: 500, color: '#B5D4F4', marginBottom: 8 }} className="flex-none">Registro en vivo</div>
          <div className="flex flex-col gap-2" style={{ flex: 1, minHeight: 0, overflowY: 'auto' }}>
            {feed.length === 0 ? <div style={{ fontSize: 12, color: '#5b7aa6' }}>Sin movimientos recientes</div>
              : feed.map((f, i) => (
                <div key={i} className="flex items-center gap-2 flex-none" style={{ borderBottom: '1px solid rgba(133,183,235,0.1)', paddingBottom: 6 }}>
      ...
      {/* Ocupación por piso */}
      <div style={{ ...PANEL, padding: '10px 14px', flex: 'none' }}>
        <div className="flex items-center justify-between" style={{ marginBottom: 6 }}>
          <span style={{ fontSize: 13, fontWeight: 500, color: '#B5D4F4' }}>Visitantes dentro por piso</span>
          <span style={{ fontSize: 10, color: '#5b7aa6' }}>Actualizado {actualizado} · refresco {REFRESCO_MS / 1000}s</span>
        </div>
          ...
          {pisos.length === 0 ? <div style={{ fontSize: 12, color: '#5b7aa6' }}>Sin visitantes activos</div>
            : pisos.map((p) => (
              <div key={p.piso} className="flex flex-col items-center gap-1" style={{ flex: 1, height: '100%', justifyContent: 'flex-end' }}>
                ...
                <div style={{ width: '100%', height: Math.round((p.n / maxPiso) * 38), background: '#2f7ccb', borderRadius: '4px 4px 0 0', boxShadow: '0 0 8px rgba(47,124,203,.4)' }} />
